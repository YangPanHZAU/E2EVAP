...
uniformsample, four_idx, get_img_gt, img_poly_to_can_poly, augment,get_cornerPoint
import numpy as np
import copy
...
class Dataset(data.Dataset):
    def __init__(self, anno_file, data_root, split, cfg):
        ...
        self.d = Douglas()
        self.parent_dir = os.path.dirname(self.data_root)
        self.edge_path = os.path.join(self.parent_dir,'ifly_edge_train_thick3')
    ...
    def __getitem__(self, index):
        ...
        ann_array=[ann_in['segmentation'][0] for ann_in in anno]
        mask_raw=polygons_to_bitmask(ann_array,height,width)

        # if 'changguang_parcel' in image_path:
        edge_image_name = os.path.basename(image_path)
        edge_raw=io.imread(os.path.join(self.edge_path,edge_image_name.replace('.jpg','.tif')))
        orig_img, inp,inp_mask,inp_edge, trans_input, trans_output, flipped, center, scale, inp_out_hw = \
            augment(
                img,mask_raw,edge_raw, self.split,
                ...
                self.cfg.data.scale, self.cfg.test.test_rescale, self.cfg.data.test_scale,self.cfg.data.train_ms_mode
            )
        ...
        data_input.update({'inp': inp})#[in_channels,512,512]
        data_input.update({'inp_mask': inp_mask})#[1,512,512]
        data_input.update({'inp_edge': inp_edge})#[1,512,512]
...
class Dataset_corner(data.Dataset):
    def __init__(self, anno_file, data_root, split, cfg):
        super(Dataset_corner, self).__init__()
        self.cfg = cfg
        self.data_root = data_root
        self.split = split
    ...
    #     img_gt_polys.append(img_gt_poly)
    #     can_gt_polys.append(can_gt_poly)
    def prepare_evolution_corner_coding(self, poly, img_gt_polys, can_gt_polys, keyPointsMask,cornerPointsMask):
        img_gt_poly = uniformsample(poly, self.cfg.data.points_per_poly)
        # idx = four_idx(img_gt_poly)
        # img_gt_poly = get_img_gt(img_gt_poly, idx)
        can_gt_poly = img_poly_to_can_poly(img_gt_poly)
        key_mask = self.get_keypoints_mask(img_gt_poly)
        cornerPoints = get_cornerPoint(poly,img_gt_poly)
        keyPointsMask.append(key_mask)
        img_gt_polys.append(img_gt_poly)
        can_gt_polys.append(can_gt_poly)
        cornerPointsMask.append(cornerPoints)
    ...
    def __getitem__(self, index):
        ...
        ann_array=[ann_in['segmentation'][0] for ann_in in anno]
        mask_raw=polygons_to_bitmask(ann_array,height,width)
        if 'San_joaquin' in image_path:
            edge_raw=io.imread(image_path.replace('train_coco','San_joaquin_2014_NAIP_edge_train_thick3'))
        if 'changguang_parcel' in image_path:
            edge_raw=io.imread(image_path.replace('train_coco','ifly_edge_train_thick3').replace('.jpg','.tif'))
        orig_img, inp,inp_mask,inp_edge, trans_input, trans_output, flipped, center, scale, inp_out_hw = \
            augment(
                img,mask_raw,edge_raw, self.split,
                self.cfg.data.data_rng, self.cfg.data.eig_val, self.cfg.data.eig_vec,
        ...
        for i in range(len(anno)):
            ...
            for j in range(len(instance_poly)):
                ...
                if h <= 1 or w <= 1:
                    continue
                self.prepare_detection(bbox, poly, ct_hm, cls_id, wh, ct_cls, ct_ind)
                self.prepare_evolution_corner_coding(poly, img_gt_polys, can_gt_polys, keyPointsMask,cornerPointsMask)
                #self.prepare_evolution(poly, img_gt_polys, can_gt_polys, keyPointsMask)
        ...
        data_input.update(evolution)
        data_input.update({'keypoints_mask': keyPointsMask})
        data_input.update({'cornepoints_mask': cornerPointsMask})
        ct_num = len(ct_ind)
        meta = {'center': center, 'scale': scale, 'img_id': image_id, 'ann': ann, 'ct_num': ct_num}
        data_input.update({'meta': meta})
        return data_input
