...
def augment(img, mask,edge, split, _data_rng, _eig_val, _eig_vec, mean, std,
            down_ratio, input_h, input_w, scale_range, scale=None, test_rescale=None, test_scale=None,tran_ms_mode=False):
    ...
    input_img_mask=np.concatenate((img,mask[:,:,None],edge[:,:,None]),axis=2)
    trans_input = get_affine_transform(center, scale, 0, [input_w, input_h])
    inp_input_img_mask = cv2.warpAffine(input_img_mask, trans_input, (input_w, input_h), flags=cv2.INTER_LINEAR)
    #inp = cv2.warpAffine(img, trans_input, (input_w, input_h), flags=cv2.INTER_LINEAR)
    #inp_mask = cv2.warpAffine(mask, trans_input, (input_w, input_h), flags=cv2.INTER_LINEAR)
    inp = inp_input_img_mask[:,:,:-2]
    orig_img = inp.copy()
    ...
    inp_mask = inp_input_img_mask[:,:,-2:-1].transpose(2, 0, 1)
    inp_edge = inp_input_img_mask[:,:,-1:].transpose(2, 0, 1)
    ...
    return orig_img, inp, inp_mask,inp_edge, trans_input, trans_output, flipped, center, scale, inp_out_hw
...
def get_cornerPoint(poly,img_poly):
    poly_len=poly.shape[0]
    corner_encode=np.zeros((img_poly.shape[0]))
    for i in range(poly_len):
        corner_code=((img_poly==poly[i,:]).astype('int').sum(axis=1)==2).astype('int')
        corner_encode+=corner_code
    PointMask=corner_encode
    return PointMask
...
# b=uniformsample(a, 8)
# c=get_cornerPoint(poly=a,img_poly=b)
# print(c)
# import numpy as np
# a=np.random.randn(4,2)
# b=uniformsample(a, 4*8)
# # print(b.shape)
# # print(b)
# idx = four_idx(b)
# b = get_img_gt(b, idx,8)
# print(a)
# print(b)
# for i in range(4):
#     dsitance=np.sum((b-a[i,:])**2,1)
#     index=np.argmin(dsitance)
#     print(index)
# encode=np.zeros((1,8))
# for i in range(3):
#     code=(b==a[i,:]).astype('int').sum(axis=1)
#     encode+=code

#b==a
# print(np.where(b==a[:,None]))
